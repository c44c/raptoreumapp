...
// raptoreum.app object instantiation
(function(scope, endpoint){
    ...
    // rtmtv object definition
    var RTMTV = (function(endpoint){
        ...
        return {
            ...
            init: function() {
                ...
                // get initial data
                this.pollRtmData(self);

                // set polling timers
                this.setTimer('netdata', function(){ self.pollRtmData(self)}, 60 * 1000);  // once per minute
            ...
            // update object with latest rtm data
            pollRtmData: function(self) {
                self.data.polling += 1;
            ...
            cacheSelectors: function(){
                ...
                s.value_fiatRtm    = $("#fiatRtm>div.v");
                s.value_fiatBTC     = $("#fiatBTC>div.v");
                s.value_rtm        = $("#rtmValue>div.v");
                s.value_rtmMonth   = $("#rtmMonth>div.v");
                s.value_rtmDay     = $("#rtmDay>div.v");
                s.value_btcMUE     = $("#btcRtm>div.v");
            ...
            redraw: function(){
                ...
                if (this.connState('now') != this.connState('last')) {
                    ...
                    if (this.connState('now') == UP){
                        ...
                        s.box.attr('title','');
                        var self = this;
                        this.setTimer('netdata', function(){ self.pollRtmData(self)}, 60 * 1000);
                    }
                    else {
                        s.body.addClass("err");
                        ...
                        // reference for callbacks
                        var self = this;
                        this.setTimer('netdata', function(){ self.pollRtmData(self)}, 15 * 1000);
                    }
                }
                ...
                switch(this.toggle) {
                    case true:
                        document.title = "Raptoreum.App - " + fmtCurrShort(data.rtm * data.fiatprice) + " " + this.data.curr;
                        break;
                    case false:
                        document.title = "Raptoreum.App - " + fmtCurrShort(data.fiatprice) + " " + this.data.curr;
                        break;
                };
                ...
                s.value_fiatRtm    .html(fmt.fiatRtm   );
                s.value_fiatBTC     .html(fmt.fiatBTC    );
                s.value_rtm        .html(fmt.rtmValue  );
                s.value_rtmMonth   .html(fmt.rtmMonth  );
                s.value_rtmDay     .html(fmt.rtmDay    );
                s.value_btcMUE     .html(fmt.btcRtm    );
            ...
            updateData: function(o) {
                ...
                this.data.mn = math.floor(this.data.rtm/1000000);
                ...
                fmt.value       = fmtCurrShort(data.rtm * data.fiatprice);
                fmt.fiatMonth   = fmtCurrShort(data.rtmdaily * 365 / 12 * data.fiatprice * data.mn);
                fmt.fiatDay     = fmtCurrShort(data.rtmdaily * data.fiatprice * data.mn);
                fmt.fiatRtm    = fmtCurr(data.fiatprice, 3);
                fmt.fiatBTC     = fmtCurrShort(data.fiatbtc);
                fmt.rtmValue   = fmtCurrShort(data.rtm);
                fmt.rtmMonth   = fmtCurrShort(data.rtmdaily * 365 / 12 * data.mn);
                fmt.rtmDay     = fmtCurrShort(data.rtmdaily * data.mn);
                fmt.btcRtm     = fmtCurr(data.btcprice, 8);
                fmt.shareMn     = fmtCurr(data.mn / data.mncount * 100, 2) + "%";
                fmt.interest    = fmtCurr(data.rtmdaily * 365/1000000 * 100, 2) + "%";
                fmt.sharesupply = fmtCurr(data.rtm/data.totalsupply * 100, 3) + "%";
...
})(this, 'https://raptoreum.app/');


// invoke rtmtv.init() on page load
$(document).ready(function(){

    // attach hash-change handler
    window.onhashchange = function(){ rtmtv.reload(); };
